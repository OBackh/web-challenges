...
function hasAnimal(animals, animalName) {
  const animalNameUp = animalName.toUpperCase();
  console.log(animalNameUp);
  const animalsUp = animals.map((animals) => {
    return animals.toUpperCase();
  });
  console.log(animalsUp);
  const searchResult = animalsUp.includes(animalNameUp);
  return searchResult;
}
